...
def write_varlist(dlim, elim, varlist, fn=None, eOp=1.5,
  ...
  if hdf5:
    ...
    with h5py.File(fn, 'w') as f:
        ...
        f.create_dataset('ratios', data=np.array([1, eOp, eOp / (1 + eOp)], dtype=ftype))
      ...
      with open(fn, 'w') as f:
        ...
        np.array([1., eOp, eOp / (1 + eOp)] + [0 for i in range(len(varlist) - 3)]).tofile(f, **opt)
        f.write('\n')
        for i, d in enumerate(varlist):
            f.write('# ' + _names[i] + '\n')
...
def run(**kwargs):
  ...
  arguments1.extend(['hydro/EOS_file_name=gamma_is_{0:.3f}.tab','hydro/EOS_file_type=ascii'])
  for i, g in enumerate(_gammas):
      arguments = [j.format(g, i) for j in arguments0]
      athena.run('hydro/athinput.sod', arguments)
...
def analyze():
  ...
  for i, g in enumerate(_gammas):
    for t in [10,26]:
        ...
        x_ascii,_,_,data_ascii = athena_read.vtk('bin/Sod_eos_hllc_ascii_{0:}.block0.out1.{1:05d}.vtk'.format(i, t))
        loc = [0,0,slice(None)]
        for var in ['rho', 'press']:
            norm = comparison.l1_norm(x_ref, data_ref[var][loc])
            diff = comparison.l1_diff(x_ref, data_ref[var][loc], x_new, data_new[var][loc]) / norm
            if diff > 1e-3 or np.isnan(diff):
              print(' '.join(map(str, ['Eos table test fail (binary). var, diff, gamma =', var, diff, g])))
              analyze_status = False
            diff = comparison.l1_diff(x_ref, data_ref[var][loc], x_ascii, data_ascii[var][loc]) / norm
            if diff > 1e-3 or np.isnan(diff):
              print(' '.join(map(str, ['Eos table test fail (ascii). var, diff, gamma =', var, diff, g])))
              analyze_status = False
