...
def analyze():
    ...
    new_rate = -new_rate
    logger.warning('[Decaying Linear Wave-3D STS]: Ref(decay_rate) = {}'.format(rate))
    logger.warning('[Decaying Linear Wave-3D STS]: New(decay_rate) = {}'.format(new_rate))
    ...
    if error_rel > 0.1:
        ...
        logger.warning('[Decaying Linear Wave-3D STS]: decay rate is within 10 percent precision')
