...
def run(**kwargs):
    for i in (256, 512):
      arguments0 = ['output1/file_type=hst', 'output1/dt=0.1',
                    ...
                    'time/cfl_number=0.3', 'time/tlim=0.5', 'time/nlim=200',
...
def analyze():
    ...
    for n in res:
      x1v,\
      v2 = np.loadtxt( "bin/visc"+str(n)+".block0.out2.00002.tab", 
      ...
      analytic=(v0/np.sqrt(2.*np.pi*sigma**2.))*\
               (1./np.sqrt(1.+(2.*nu*t0/sigma**2.)))*\
                np.exp(-(x1v[8*N:8*N+N]**2.)/(2.*sigma**2.*(1.+(2.*nu*t0/sigma**2.))))
    ...
    flag = True
    if conv > -0.93:
        print('[Viscous Diffusion STS]: STS Scheme NOT Converging at ~1st order.')
        flag = False
    else:
        print('[Viscous Diffusion STS]: STS Scheme Converging at ~1st order.')
