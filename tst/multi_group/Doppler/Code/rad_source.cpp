...
#pragma omp simd
        ...
        if(nfreq == 1){
         ...
         // Add compton scattering
          ...
          for(int ifr=0; ifr<nfreq; ++ifr)
             for(int n=0; n<nang; ++n)
               pmb->user_out_var(ifr*nang+n,k,j,i) = ir_shift_(ifr*nang+n);
...
void RadIntegrator::AddIMSourceTerms(MeshBlock *pmb, AthenaArray<Real> &u, 
                       AthenaArray<Real> &rad_source)
{
  ...
  for(int k=ks; k<=ke; ++k){
    for(int j=js; j<=je; ++j){
      for(int i=is; i<=ie; ++i){
        u(IEN,k,j,i) += rad_source(0,k,j,i);
