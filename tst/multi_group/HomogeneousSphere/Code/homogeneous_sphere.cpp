        ...
        for (int i=is; i<=ie; ++i) {
          ...
          if(prad->nfreq == 1){
            ...
            for(int ifr=0; ifr<prad->nfreq; ++ifr){
              Real er = 1.0;
              if(ifr == prad->nfreq-1)
                er = 1.0 - prad->FitBlackBody(prad->nu_grid(ifr));
              else
                er = prad->BlackBodySpec(prad->nu_grid(ifr), prad->nu_grid(ifr+1));

              for(int n=0; n<prad->nang; ++n)
                prad->ir(k,j,i,ifr*prad->nang+n) = er;
            }
          ...
          for (int ifr=0; ifr < nfreq; ++ifr){
            ...
            if (pcoord->x1v(i) < 1.0) {
                ...
                prad->sigma_planck(k,j,i,ifr) = 10.0;
            }
            else {
      ...
      for (int i=1; i<=ngh; ++i) {
        if(prad->nfreq == 1){
          ...
          for(int ifr=0; ifr<prad->nfreq; ++ifr){
            ...
            for(int n=0; n<prad->nang; ++n)
              prad->ir(k,j,is-i,ifr*prad->nang+n) = er;
          }
