    ...
    for(int k=0; k<ncells3; ++k) {
      for (int j=0; j<ncells2; ++j) {
        for (int i=0; i<ncells1; ++i) {
          ...
          prad->sigma_s(k,j,i,1) = 0.0;
          prad->sigma_a(k,j,i,1) = sigma0 * phydro->u(IDN,k,j,i);
          prad->sigma_ae(k,j,i,1) = prad->sigma_a(k,j,i,1);          
...
void FFOpacity(MeshBlock *pmb, AthenaArray<Real> &prim)
{
  ...
  for (int i=il; i<=iu; ++i) {
    ...
    prad->sigma_ae(k,j,i,0) = prad->sigma_a(k,j,i,0);
    prad->sigma_planck(k,j,i,0) = 0.0;

    prad->sigma_s(k,j,i,1) = 0.0;
    prad->sigma_a(k,j,i,1) = sigma0 * rho;;
    prad->sigma_ae(k,j,i,1) = prad->sigma_a(k,j,i,1);
