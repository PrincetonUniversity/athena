...
const Real inject_tr = 10.0;
const Real vel = 0.0;
const Real sigma0 = 100.0;
const Real nu_width = 0.3;
const Real nu_center = 1.0;
    ...
    for(int k=0; k<ncells3; ++k)
     for(int j=0; j<ncells2; ++j)
       for(int i=0; i<ncells1; ++i){
          ...
          Real pos_coef = (x1-0.5)/0.1;
          for (int ifr=0; ifr < nfreq-1; ++ifr){
            Real nu=prad->nu_cen(ifr);
            Real nu_coef = (nu-nu_center)/nu_width;
            prad->sigma_s(k,j,i,ifr) = 0.0;
            prad->sigma_a(k,j,i,ifr) = sigma0 * exp(-nu_coef*nu_coef) * exp(-pos_coef*pos_coef);
        ...
        for(int ifr=0; ifr<nfreq; ++ifr){
          for(int n=0; n<nang; ++n){
            if(prad->mu(0,k,j,is-i,n) > 0.0){
              ir(k,j,is-i,nang*ifr+n) = blackbody; 
