...
//  Real n_nusq_last_bin = pmy_rad->ConvertBBJNNu2(j_nu[nfreq-1],nu_grid[nfreq-1]);
  ...
  for(int ifr=1; ifr<nfreq-1; ++ifr){
    ...
    com_b_face_coef[ifr] = a_coef*(tdnu_face+(1.0+0.0 * n_face)*(1.0-delta_coef[ifr]));
    com_d_face_coef[ifr] = a_coef*((1+0.0 *n_face)*delta_coef[ifr]-tdnu_face);
  }
...
//  Real nf_last=1.0/(\lambda exp(nu_tr)-1.0);
  ...
  if(nf_last < n_nu[nfreq-2]){
    flux_last_explicit=a_coef_last*(nf_last*(1.0+0.0*nf_last)
...
Real RadIntegrator::QuasiEqSol(Real &tgas, Real &tot_n)
{
  ...
  if((n_t3 < 2.3739) && (n_t3 > 0.6932)){
    eq_sol_c = 1.948 * pow(n_t3, -1.016) + 0.1907;
  }else if(n_t3 <= 0.6932){
