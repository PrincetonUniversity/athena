...
#pragma omp simd simdlen(SIMD_WIDTH)
        for (int i=il-1; i<=(iu+1); ++i) {
          ...
          if (SIGN(qa) == SIGN(qb) && SIGN(qa) == SIGN(qc)) {
            qd = SIGN(qa)* std::min(C2*std::fabs(qb),std::min(C2*std::fabs(qc),std::fabs(qa)));
          }
...
#pragma omp simd simdlen(SIMD_WIDTH)
        for (int i=il-1; i<=iu; ++i) {
          ...
          qb = std::max(std::max(std::fabs(q(n,i)),std::fabs(q_ip1(n,i))), std::fabs(q_ip2(n,i)));
      ...
      // Note, the Mignone limiter does not check for cell-averaged extrema:
        ...
        for (int i=il-1; i<=iu; ++i) {
          ...
          if (qa <= 0.0) { // Local extrema detected
            ...
            // Overshoot i-1/2,R / i,(-) state
            if (std::fabs(dqf_minus(i)) >= prec->hplus_ratio_i(i)*std::fabs(dqf_plus(i))) {
              qminus(i) = q(n,i) - prec->hplus_ratio_i(i)*dqf_plus(i);
            }
            // Overshoot i+1/2,L / i,(+) state
            if (std::fabs(dqf_plus(i)) >= prec->hminus_ratio_i(i)*std::fabs(dqf_minus(i))) {
...
#pragma omp simd simdlen(SIMD_WIDTH)
        for (int i=il; i<=iu; ++i) {
          ...
          qb = std::max(std::max(std::fabs(q(n,i)),std::fabs(q_jp1(n,i))), std::fabs(q_jp2(n,i)));
      ...
      // Note, the Mignone limiter does not check for cell-averaged extrema:
        ...
        for (int i=il; i<=iu; ++i) {
          ...
          if (qa <= 0.0) { // Local extrema detected
            ...
            // Overshoot j-1/2,R / j,(-) state
            if (std::fabs(dqf_minus(i)) >= prec->hplus_ratio_j(j)*std::fabs(dqf_plus(i))) {
              qminus(i) = q(n,i) - prec->hplus_ratio_j(j)*dqf_plus(i);
            }
            // Overshoot j+1/2,L / j,(+) state
            if (std::fabs(dqf_plus(i)) >= prec->hminus_ratio_j(j)*std::fabs(dqf_minus(i))) {
...
#pragma omp simd simdlen(SIMD_WIDTH)
        for (int i=il; i<=iu; ++i) {
          ...
          qb = std::max(std::max(std::fabs(q(n,i)),std::fabs(q_kp1(n,i))), std::fabs(q_kp2(n,i)));
      ...
      // Note, the Mignone limiter does not check for cell-averaged extrema:
        ...
        for (int i=il; i<=iu; ++i) {
          ...
          if (qa <= 0.0) { // Local extrema detected
            ...
            // Overshoot k-1/2,R / k,(-) state
            if (std::fabs(dqf_minus(i)) >= prec->hplus_ratio_k(k)*std::fabs(dqf_plus(i))) {
              qminus(i) = q(n,i) - prec->hplus_ratio_k(k)*dqf_plus(i);
            }
            // Overshoot k+1/2,L / k,(+) state
            if (std::fabs(dqf_plus(i)) >= prec->hminus_ratio_k(k)*std::fabs(dqf_minus(i))) {
