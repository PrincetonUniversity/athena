...
TaskStatus TimeIntegratorTaskList::CalculateHydroFlux(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    if ((stage == 1) && (integrator == "vl2")) {
      // TODO(felker): check safety of passing unallocated b_fc, bcc_center when
...
TaskStatus TimeIntegratorTaskList::IntegrateField(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    if (stage==4 && integrator == "ssprk5_4") {
      ...
      pmb->WeightedAve(pf->b2, pf->b1, pf->b2, ave_wghts);
      pf->CT(beta, pf->b2);
    }
...
TaskStatus TimeIntegratorTaskList::Primitives(MeshBlock *pmb, int stage) {
  ...
  if (stage <= nstages) {
    ...
    if (order == 4) {
      ...
      Real time = 0.0;
      pmb->pbval->ApplyPhysicalBoundariesConserved(ph->w1, ph->u, pf->b, pf->bcc, time,
                                                   0.0);
    }
    ...
    pmb->peos->ConservedToPrimitive(ph->u, ph->w, pf->b, ph->w1, pf->bcc, pmb->pcoord,
                                    il, iu, jl, ju, kl, ku);
    if (NSCALARS > 0) {
