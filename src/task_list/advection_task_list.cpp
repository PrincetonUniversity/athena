...
//========================================================================================
//! \file advection_task_list.cpp
//  \brief time integrator for the advection equation (based on time_integrator.cpp)
...
//----------------------------------------------------------------------------------------
//  AdvectionIntegratorTaskList constructor

AdvectionIntegratorTaskList::AdvectionIntegratorTaskList(ParameterInput *pin, Mesh *pm){
  ...
  // Now assemble list of tasks for each stage of advection integrator
  {using namespace AdvectionIntegratorTaskNames;
    AddTask(CALC_ADVECTIONRHS, NONE);

    AddTask(INT_ADVECTION, CALC_ADVECTIONRHS);

    AddTask(SEND_ADVECTION, INT_ADVECTION);
    AddTask(RECV_ADVECTION, NONE);

    AddTask(SETB_ADVECTION, (RECV_ADVECTION|INT_ADVECTION));

    if (pm->multilevel) { // SMR or AMR
      AddTask(PROLONG, (SEND_ADVECTION|SETB_ADVECTION));
      AddTask(PHY_BVAL, PROLONG);
    } else {
      AddTask(PHY_BVAL, SETB_ADVECTION);
    }
...
void AdvectionIntegratorTaskList::AddTask(const TaskID& id, const TaskID& dep) {
    task_list_[ntasks].task_id = id;
    task_list_[ntasks].dependency = dep;

    using namespace AdvectionIntegratorTaskNames; // NOLINT (build/namespace)
    if (id == CLEAR_ALLBND) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::ClearAllBoundary);
      task_list_[ntasks].lb_time = false;
    } else if (id == CALC_ADVECTIONRHS) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::CalculateAdvectionRHS);
      task_list_[ntasks].lb_time = true;
    } else if (id == INT_ADVECTION) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::IntegrateAdvection);
      task_list_[ntasks].lb_time = true;
    } else if (id == SEND_ADVECTION) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::SendAdvection);
      task_list_[ntasks].lb_time = true;
    } else if (id == RECV_ADVECTION) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::ReceiveAdvection);
      task_list_[ntasks].lb_time = false;
    } else if (id == SETB_ADVECTION) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::SetBoundariesAdvection);
      task_list_[ntasks].lb_time = true;
    } else if (id == PROLONG) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::Prolongation);
      task_list_[ntasks].lb_time = true;
    } else if (id == PHY_BVAL) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::PhysicalBoundary);
      task_list_[ntasks].lb_time = true;
    } else if (id == USERWORK) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::UserWork);
      task_list_[ntasks].lb_time = true;
    } else if (id == NEW_DT) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::NewBlockTimeStep);
      task_list_[ntasks].lb_time = true;
    } else if (id == FLAG_AMR) {
      task_list_[ntasks].TaskFunc=
        static_cast<TaskStatus (TaskList::*)(MeshBlock*,int)>
        (&AdvectionIntegratorTaskList::CheckRefinement);
      task_list_[ntasks].lb_time = true;
    } else {
      std::stringstream msg;
...
void AdvectionIntegratorTaskList::StartupTaskList(MeshBlock *pmb, int stage) {
  if (stage == 1) {
...
TaskStatus AdvectionIntegratorTaskList::ClearAllBoundary(MeshBlock *pmb, int stage) {
...
TaskStatus AdvectionIntegratorTaskList::CalculateAdvectionRHS(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
    pmb->padv->AdvectionRHS(pmb->padv->u);

    // application of Sommerfeld boundary conditions
    pmb->padv->AdvectionBoundaryRHS(pmb->padv->u);
...
TaskStatus AdvectionIntegratorTaskList::IntegrateAdvection(MeshBlock *pmb, int stage) {
  Advection *padv = pmb->padv;
  ...
  if (stage <= nstages) {
    ...
    ave_wghts[2] = 0.0;
    pmb->WeightedAve(padv->u1, padv->u, padv->u2, ave_wghts);

    ave_wghts[0] = stage_wghts[stage-1].gamma_1;
    ave_wghts[1] = stage_wghts[stage-1].gamma_2;
    ave_wghts[2] = stage_wghts[stage-1].gamma_3;

    pmb->WeightedAve(padv->u, padv->u1, padv->u2, ave_wghts);

    padv->AddAdvectionRHS(stage_wghts[stage-1].beta, padv->u);
...
TaskStatus AdvectionIntegratorTaskList::SendAdvection(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
...
TaskStatus AdvectionIntegratorTaskList::ReceiveAdvection(MeshBlock *pmb, int stage) {
  bool ret;
  if (stage <= nstages) {
    ret = pmb->padv->ubvar.ReceiveBoundaryBuffers();
  } else {
    return TaskStatus::fail;
  }
  if (ret) {
...
TaskStatus AdvectionIntegratorTaskList::SetBoundariesAdvection(MeshBlock *pmb, int stage) {
  if (stage <= nstages) {
    pmb->padv->ubvar.SetBoundaries();
    return TaskStatus::success;
  }
  return TaskStatus::fail;
}
...
TaskStatus AdvectionIntegratorTaskList::Prolongation(MeshBlock *pmb, int stage) {
...
TaskStatus AdvectionIntegratorTaskList::PhysicalBoundary(MeshBlock *pmb, int stage) {
  BoundaryValues *pbval = pmb->pbval;
...
TaskStatus AdvectionIntegratorTaskList::UserWork(MeshBlock *pmb, int stage) {
  if (stage != nstages) return TaskStatus::success; // only do on last stage

  pmb->AdvectionUserWorkInLoop();
  return TaskStatus::success;
}

TaskStatus AdvectionIntegratorTaskList::NewBlockTimeStep(MeshBlock *pmb, int stage) {
  if (stage != nstages) return TaskStatus::success; // only do on last stage

  pmb->padv->NewBlockTimeStep();
  return TaskStatus::success;
}


TaskStatus AdvectionIntegratorTaskList::CheckRefinement(MeshBlock *pmb, int stage) {
  if (stage != nstages) return TaskStatus::success; // only do on last stage
