...
// Declarations
static void set_state(AthenaArray<Real> &cons, int i, int j, int k, Real rho, Real pgas,
    Real vx, Real vy, Real vz, Real gamma_adi, Real gamma_adi_red);
...
void Fluid::InitProblem(ParameterInput *pin)
{
  ...
  {
    case 1:
      min_bound = pb->pparent_domain->pparent_mesh->mesh_size.x1min;
      max_bound = pb->pparent_domain->pparent_mesh->mesh_size.x1max;
      break;
    case 2:
      min_bound = pb->pparent_domain->pparent_mesh->mesh_size.x2min;
      max_bound = pb->pparent_domain->pparent_mesh->mesh_size.x2max;
      break;
    case 3:
      min_bound = pb->pparent_domain->pparent_mesh->mesh_size.x3min;
      max_bound = pb->pparent_domain->pparent_mesh->mesh_size.x3max;
      break;
    default:
  ...
  for (int k = ks; k <= ke; k++)
    for (int j = js; j <= je; j++)
      for (int i = is; i <= ie; i++)
      {
        ...
        if (left_side)
          set_state(u, i, j, k, rho_left, pgas_left, vx_left, vy_left, vz_left,
              gamma_adi, gamma_adi_red);
        else
          set_state(u, i, j, k, rho_right, pgas_right, vx_right, vy_right, vz_right,
              gamma_adi, gamma_adi_red);
      }
  return;
}
    ...
    Real vx, Real vy, Real vz, Real gamma_adi, Real gamma_adi_red)
{
  ...
  cons(IDN,k,j,i) = sqrt(gamma_lor_sq) * rho;
