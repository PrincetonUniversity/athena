...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int j=js; j<=je; ++j) {
    for (int i=is; i<=ie; ++i) {
      ...
      Real shock_xpos_btm = 0.1666666666 + pcoord->x2f(j  )/std::sqrt(static_cast<Real>(3.0));
      Real shock_xpos_top = 0.1666666666 + pcoord->x2f(j+1)/std::sqrt(static_cast<Real>(3.0));
