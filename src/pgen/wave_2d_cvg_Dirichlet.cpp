...
//========================================================================================
//! \file wave_2d_cvg_Dirichlet.cpp
//  \brief Problem generator for two-dimensional Dirichlet problem wave equation
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  int const num_M = pwave->A_.GetDim2();
  int const num_N = pwave->A_.GetDim1();
      ...
      for(int i = il; i <= iu; ++i) {
        ...
        for(int ix_n=1; ix_n<=num_N; ++ix_n)
          for(int ix_m=1; ix_m<=num_M; ++ix_m) {
            Real const beta_ix_mn = PI * SQRT(SQR(ix_m / Lx1) + SQR(ix_n / Lx2));
            ...
            Real const A = pwave->A_(ix_m-1, ix_n-1);
            Real const B = pwave->B_(ix_m-1, ix_n-1);

            pwave->u(0,k,j,i) += A * sin_beta_x * sin_beta_y;
            pwave->u(1,k,j,i) += B * c * beta_ix_mn *
              sin_beta_x * sin_beta_y;
...
void MeshBlock::WaveUserWorkInLoop() {
      ...
      for(int i = il; i <= iu; ++i) {
        ...
        for(int ix_n=1; ix_n<=num_N; ++ix_n)
          for(int ix_m=1; ix_m<=num_M; ++ix_m) {
            ...
            Real const cos_beta_ct = cos(beta_ix_mn * c * t);
            Real const sin_beta_ct = sin(beta_ix_mn * c * t);
            ...
            pwave->exact(k,j,i) += (A * cos_beta_ct +
                                    B * sin_beta_ct) * sin_beta_x * sin_beta_y;
  ...
  if (max_err > 0.1) {
    printf("pwave->u:\n");
    pwave->u.print_all("%1.3f");

    printf("pwave->coarse_u_:\n");
    pwave->coarse_u_.print_all("%1.3f");

    printf("pwave->exact:\n");
    pwave->exact.print_all("%1.3f");

    printf("pwave->error:\n");
    pwave->error.print_all("%1.3f");
    // if (gid > 40)
    Q();
  }
      ...
      for(int i = il; i <= iu; ++i) {
        ...
        for(int ix_n=1; ix_n<=num_N; ++ix_n)
          for(int ix_m=1; ix_m<=num_M; ++ix_m) {
            ...
            Real const dt_cos_beta_ct = -beta_ix_mn * c * sin(beta_ix_mn * c * t);
            Real const dt_sin_beta_ct = beta_ix_mn * c * cos(beta_ix_mn * c * t);
            ...
            u_1 += \
              (A * dt_cos_beta_ct +
               B * dt_sin_beta_ct) * sin_beta_x * sin_beta_y;
