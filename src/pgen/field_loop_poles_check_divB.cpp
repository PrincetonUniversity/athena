...
Real DivergenceBvol(MeshBlock *pmb, int iout);
Real AbsDivergenceB(MeshBlock *pmb, int iout);
Real AbsDivergenceBvol(MeshBlock *pmb, int iout);
Real wDivergenceB(MeshBlock *pmb, int iout);
Real wAbsDivergenceB(MeshBlock *pmb, int out);
Real wAbsDivergenceB2(MeshBlock *pmb, int out);
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (MAGNETIC_FIELDS_ENABLED){
    ...
    EnrollUserHistoryOutput(0, AbsDivergenceB, "absdivB");
    EnrollUserHistoryOutput(1, AbsDivergenceBvol, "absdivBvol");
    EnrollUserHistoryOutput(2, DivergenceB, "divB");
    EnrollUserHistoryOutput(3, DivergenceBvol, "divBvol");
    EnrollUserHistoryOutput(4, wAbsDivergenceB, "wabsdivB");
    EnrollUserHistoryOutput(5, wDivergenceB, "wdivB");
    EnrollUserHistoryOutput(6, wAbsDivergenceB2, "wabsdivB2");
    EnrollUserHistoryOutput(7, ncells, "ncells");
    EnrollUserHistoryOutput(8, Ncells, "Ncells");
  }
  return;
}
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin){
  if (MAGNETIC_FIELDS_ENABLED){
    ...
    SetUserOutputVariableName(0, "divB");
    SetUserOutputVariableName(1, "divBvol");
    SetUserOutputVariableName(2, "wabsdivB");
    SetUserOutputVariableName(3, "wabsdivB2");
...
int RefinementCondition2(MeshBlock *pmb){
  ...
  for (int k=pmb->ks; k<=pmb->ke; k++) {
	...
	if (pmb->pmy_mesh->ncycle==30){
	  Real bmag=std::sqrt(SQR(bc(IB1,k,j,i))+SQR(bc(IB2,k,j,i))+SQR(bc(IB3,k,j,i)));
	  if (bmag>=1.0e-3*b0){
	    refflag+=1.0;
	  }
	}
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin){
  ...
  if (MAGNETIC_FIELDS_ENABLED){
	...
	for(int i=is; i<=ie; i++){   
	  ...
	  Real divb=(face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
              +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
		     +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i))/cellvol; 
	  Real divbvol=(face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
              +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
			+face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i));
	  user_out_var(0,k,j,i) = divb;
	  user_out_var(1,k,j,i) = divbvol;
	  
	  Real bmag=std::sqrt(SQR(pfield->bcc(IB1,k,j,i))+SQR(pfield->bcc(IB2,k,j,i))+SQR(pfield->bcc(IB3,k,j,i)));
	  if (bmag!=0.0){
	    Real dl=std::min(std::min(pcoord->GetEdge1Length(k,j,i),pcoord->GetEdge2Length(k,j,i)),pcoord->GetEdge3Length(k,j,i));
	    Real wabsdivb=fabs(((face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
			 +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
			 +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i)))/cellvol)/(bmag/dl);
	    Real bmagdl=std::sqrt(SQR(pfield->bcc(IB1,k,j,i)/pcoord->GetEdge1Length(k,j,i))+
                                 SQR(pfield->bcc(IB2,k,j,i)/pcoord->GetEdge2Length(k,j,i))+
                                 SQR(pfield->bcc(IB3,k,j,i)/pcoord->GetEdge3Length(k,j,i)));
	    Real wabsdivb2=fabs(((face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
			 +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
			 +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i)))/cellvol)/(bmagdl);
	    user_out_var(2,k,j,i) = wabsdivb;
	    user_out_var(3,k,j,i) = wabsdivb2;
	  }else{
	    user_out_var(2,k,j,i) = 1.0e-300;
	    user_out_var(3,k,j,i) = 1.0e-300;
	  }
...
//Absolute value of the divergence of B
Real AbsDivergenceB(MeshBlock *pmb, int iout){
  Real divb=0;
  ...
  for(int k=ks; k<=ke; k++) {
	...
	Real cellvol=pmb->pcoord->GetCellVolume(k,j,i);
        divb+=fabs(((face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
              +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
	      +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i)))/cellvol);
      }
    }
...
//Absolute value of divB weighted by B field magnitude and smallest cell size
Real wAbsDivergenceB(MeshBlock *pmb, int iout){
  ...
  for(int k=ks; k<=ke; k++) {
	...
	Real bmag=std::sqrt(SQR(pfield->bcc(IB1,k,j,i))+SQR(pfield->bcc(IB2,k,j,i))+SQR(pfield->bcc(IB3,k,j,i)));
	if (bmag!=0.0){
	  Real dl=std::min(std::min(pmb->pcoord->GetEdge1Length(k,j,i),pmb->pcoord->GetEdge2Length(k,j,i)),
...
//Absolute value of divB weighted by the B field magnitudes and edge lengths in each direction
Real wAbsDivergenceB2(MeshBlock *pmb, int iout){
  Real divb=0;
  int is=pmb->is, ie=pmb->ie, js=pmb->js, je=pmb->je, ks=pmb->ks, ke=pmb->ke;
  AthenaArray<Real> face1, face2p, face2m, face3p, face3m;
  Field *pfield = pmb->pfield;
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      pmb->pcoord->Face3Area(k,   j,   is, ie,   face3m);
      for(int i=is; i<=ie; i++) {
	Real bmagdl=std::sqrt(SQR(pfield->bcc(IB1,k,j,i)/pmb->pcoord->GetEdge1Length(k,j,i))
                         +SQR(pfield->bcc(IB2,k,j,i)/pmb->pcoord->GetEdge2Length(k,j,i))
                         +SQR(pfield->bcc(IB3,k,j,i)/pmb->pcoord->GetEdge3Length(k,j,i)));
	if (bmagdl!=0.0){
	  Real cellvol=pmb->pcoord->GetCellVolume(k,j,i);
	  divb+=fabs(((face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
		       +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
...
Real DivergenceB(MeshBlock *pmb, int iout){
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      for(int i=is; i<=ie; i++) {
	Real cellvol=pmb->pcoord->GetCellVolume(k,j,i);
        divb+=(face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
              +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
	       +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i))/cellvol;
      }
    }
...
//Divergence of B weighted by B field magnitude and the smallest cell size
Real wDivergenceB(MeshBlock *pmb, int iout){
  Real divb=0;
  int is=pmb->is, ie=pmb->ie, js=pmb->js, je=pmb->je, ks=pmb->ks, ke=pmb->ke;
  AthenaArray<Real> face1, face2p, face2m, face3p, face3m;
...
//Absolute value of the volume-integrated divergence of B
Real AbsDivergenceBvol(MeshBlock *pmb, int iout){
  Real divbvol=0;
  int is=pmb->is, ie=pmb->ie, js=pmb->js, je=pmb->je, ks=pmb->ks, ke=pmb->ke;
  AthenaArray<Real> face1, face2p, face2m, face3p, face3m;
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      pmb->pcoord->Face3Area(k,   j,   is, ie,   face3m);
      for(int i=is; i<=ie; i++) {
        divbvol+=fabs(face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
              +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
		      +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i));
      }
    }
...
//Volume-integrated divergence of B
Real DivergenceBvol(MeshBlock *pmb, int iout){
  Real divbvol=0;
  int is=pmb->is, ie=pmb->ie, js=pmb->js, je=pmb->je, ks=pmb->ks, ke=pmb->ke;
  AthenaArray<Real> face1, face2p, face2m, face3p, face3m;
  ...
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      ...
      pmb->pcoord->Face3Area(k,   j,   is, ie,   face3m);
      for(int i=is; i<=ie; i++) {
        divbvol+=(face1(i+1)*pfield->b.x1f(k,j,i+1)-face1(i)*pfield->b.x1f(k,j,i)
              +face2p(i)*pfield->b.x2f(k,j+1,i)-face2m(i)*pfield->b.x2f(k,j,i)
		  +face3p(i)*pfield->b.x3f(k+1,j,i)-face3m(i)*pfield->b.x3f(k,j,i)); 
      }
    }
...
//Number of cells used in the computation of the divergence of B
