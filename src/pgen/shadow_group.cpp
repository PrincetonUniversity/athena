...
void TwoBeams(MeshBlock *pmb, Coordinates *pco, Radiation *prad, 
          const AthenaArray<Real> &w, FaceField &bcc, 
          AthenaArray<Real> &ir,
          Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh)
{
        ...
        for(int ifr=0; ifr<nfreq; ++ifr){
          if(ifr == 1){
            for(int l=0; l<noct; ++l){
              for(int n=0; n<ang_oct; ++n){
                int n_ang=l*ang_oct + n;
                  if(l ==0 || l == 2){
                    if(n == 3)
                      ir(k,j,is-i, n_ang+ifr*nang) = 0.5*jr/prad->wmu(n_ang);
                    else
            ...
            for(int n=0; n<nang; ++n){
              ...
              if(miux > 0.0)
                ir(k,j,is-i, n+ifr*nang) = 0.0;
              else
                ir(k,j,is-i, n+ifr*nang) = ir(k,j,is, n+ifr*nang);
            }  
...
void FFOpacity(MeshBlock *pmb, AthenaArray<Real> &prim)
{
  ...
  for (int i=il; i<=iu; ++i) {
    ...
    prad->sigma_s(k,j,i,0) = 0.0;
    prad->sigma_a(k,j,i,0) = sigma0 * rho * rho * tpower;
    prad->sigma_pe(k,j,i,0) = prad->sigma_a(k,j,i,0);
    prad->sigma_p(k,j,i,0) = prad->sigma_pe(k,j,i,0);

    prad->sigma_a(k,j,i,1) = 0.0;
    prad->sigma_s(k,j,i,1) = sigma0 * rho;;
    prad->sigma_pe(k,j,i,1) = prad->sigma_a(k,j,i,1);
    prad->sigma_p(k,j,i,1) = prad->sigma_pe(k,j,i,1);
