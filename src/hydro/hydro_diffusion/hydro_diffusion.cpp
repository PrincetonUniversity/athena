...
#pragma omp simd
      ...
      if (nu_iso > 0.0) {
#pragma omp simd
        for (int i=il; i<=iu; ++i) nu_t(i) += nu(DiffType::iso,k,j,i);
      }
      if (nu_aniso > 0.0) {
#pragma omp simd
        for (int i=il; i<=iu; ++i) nu_t(i) += nu(DiffType::aniso,k,j,i);
      }
      if (kappa_iso > 0.0) {
#pragma omp simd
        for (int i=il; i<=iu; ++i) kappa_t(i) += kappa(DiffType::iso,k,j,i);
      }
      if (kappa_aniso > 0.0) {
#pragma omp simd
        for (int i=il; i<=iu; ++i) kappa_t(i) += kappa(DiffType::aniso,k,j,i);
      }
