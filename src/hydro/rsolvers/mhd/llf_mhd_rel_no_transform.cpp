...
#pragma omp simd simdlen(SIMD_WIDTH)
	  ...
	  for (int n = 0; n < NHYDRO; ++n) {
	    flux(n,k,j,ipm) =
	      0.5 * (flux_l[n][m] + flux_r[n][m] - lambda * (cons_r[n][m] - cons_l[n][m]));
	  }
	  ey(k,j,ipm) =
            -0.5 * (flux_l[IBY][m] + flux_r[IBY][m] - lambda * (cons_r[IBY][m] - cons_l[IBY][m]));
	  ez(k,j,ipm) =
            0.5 * (flux_l[IBZ][m] + flux_r[IBZ][m] - lambda * (cons_r[IBZ][m] - cons_l[IBZ][m]));
	}
      }
    }
  }
