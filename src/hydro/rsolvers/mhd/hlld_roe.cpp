...
void Hydro::RiemannSolver(const int k, const int j, const int il, const int iu,
  ...
  Real wli[(NWAVE)],wri[(NWAVE)],wroe[(NWAVE)]; // L/R states, primitive variables (input)
...
#pragma omp simd simdlen(SIMD_WIDTH) private(wli,wri,spd,flxi)
  for (int i=il; i<=iu; ++i) {
    ...
    //--- Step 1b.  Compute Roe-averaged data from left- and right-states
    ...
    el += 0.5*wli[IDN]*(SQR(wli[IVX])+SQR(wli[IVY])+SQR(wli[IVZ])) +pbl;
    er += 0.5*wri[IDN]*(SQR(wri[IVX])+SQR(wri[IVY])+SQR(wri[IVZ])) +pbr;
    ...
    Real q = hroe - 0.5*(SQR(wroe[IVX]) + SQR(wroe[IVY]) + SQR(wroe[IVZ])
                         + (bxsq + SQR(wroe[IBY]) + SQR(wroe[IBZ])) / wroe[IDN]);
    Real asq;
    if (GENERAL_EOS) {
      asq = (q < 0.0) ? 0.0 : pmy_block->peos->RiemannAsq(wroe[IDN], q);
    } else { //not general EOS
      asq = (q < 0.0) ? 0.0 : gm1*q;
    }
    asq *= wroe[IDN]; // Actually rho*asq
    Real ct2 = (SQR(wroe[IBY]) + SQR(wroe[IBZ]));
    Real qsq = bxsq + ct2 + asq;
    Real tmp = bxsq + ct2 - asq;
    Real cfroe = std::sqrt(0.5*(qsq + std::sqrt(tmp*tmp + 4.0*asq*ct2))/wroe[IDN]);

    Real cfl = pmy_block->peos->FastMagnetosonicSpeed(wli,bxi);
    Real cfr = pmy_block->peos->FastMagnetosonicSpeed(wri,bxi);

    spd[0] = std::min((wroe[IVX] - cfroe),(wli[IVX] - cfl));
    spd[4] = std::max((wroe[IVX] + cfroe),(wri[IVX] + cfr));
    ...
    sqrtdl = std::sqrt(ulst.d);
    sqrtdr = std::sqrt(urst.d);
