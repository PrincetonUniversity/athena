...
#pragma omp simd simdlen(SIMD_WIDTH)
	...
	for (int n = 0; n < NWAVE; ++n) {
	  cons(n,ipm) = 0.5 * (cons_r[n][m] + cons_l[n][m] + (flux_l[n][m] - flux_r[n][m]) / lambda);
	}
      }

      // Set fluxes
      for (int n = 0; n < NHYDRO; ++n) {
	flux(n,k,j,ipm) = 0.5 * (flux_l[n][m] + flux_r[n][m] - lambda * (cons_r[n][m] - cons_l[n][m]));
      }
      ey(k,j,ipm) = -0.5 * (flux_l[IBY][m] + flux_r[IBY][m] - lambda * (cons_r[IBY][m] - cons_l[IBY][m]));
      ez(k,j,ipm) = 0.5 * (flux_l[IBZ][m] + flux_r[IBZ][m] - lambda * (cons_r[IBZ][m] - cons_l[IBZ][m]));
    }
  }
...
#pragma omp simd simdlen(SIMD_WIDTH)
	...
	g30 = gi(I03,ipm), g31 = gi(I13,ipm), g32 = gi(I23,ipm), g33 = gi(I33,ipm);
      ...
      Real bb2_l = bb(k,j,ipm);
      Real bb3_l = prim_l(IBY,k,j,ipm);
      Real bb1_l = prim_l(IBZ,k,j,ipm);
      ...
      Real bb2_r = bb(k,j,ipm);
      Real bb3_r = prim_r(IBY,k,j,ipm);
      Real bb1_r = prim_r(IBZ,k,j,ipm);
	...
	+ ucon_r[3] * (g_31*bb1_r + g_32*bb2_r + g_33*bb3_r);
      ...
      cons_l[IBY][m] = bcon_l[IVZ] * ucon_l[0] - bcon_l[0] * ucon_l[IVZ];
      cons_l[IBZ][m] = bcon_l[IVX] * ucon_l[0] - bcon_l[0] * ucon_l[IVX];

      // Calculate fluxes in L region
      // (rho u^i, T^i_\mu, and *F^{ji}, where i = IVY and j != IVY)
      flux_l[IDN][m] = rho_l * ucon_l[IVY];
      flux_l[IEN][m] = wtot_l * ucon_l[IVY] * ucov_l[0] - bcon_l[IVY] * bcov_l[0];
      flux_l[IVX][m] = wtot_l * ucon_l[IVY] * ucov_l[1] - bcon_l[IVY] * bcov_l[1];
      flux_l[IVY][m] = wtot_l * ucon_l[IVY] * ucov_l[2] - bcon_l[IVY] * bcov_l[2];
      flux_l[IVZ][m] = wtot_l * ucon_l[IVY] * ucov_l[3] - bcon_l[IVY] * bcov_l[3];
      flux_l[IVY][m] += ptot_l;
      flux_l[IBY][m] = bcon_l[IVZ] * ucon_l[IVY] - bcon_l[IVY] * ucon_l[IVZ];
      flux_l[IBZ][m] = bcon_l[IVX] * ucon_l[IVY] - bcon_l[IVY] * ucon_l[IVX];
      ...
      cons_r[IBY][m] = bcon_r[IVZ] * ucon_r[0] - bcon_r[0] * ucon_r[IVZ];
      cons_r[IBZ][m] = bcon_r[IVX] * ucon_r[0] - bcon_r[0] * ucon_r[IVX];

      // Calculate fluxes in R region
      // (rho u^i, T^i_\mu, and *F^{ji}, where i = IVY and j != IVY)
      flux_r[IDN][m] = rho_r * ucon_r[IVY];
      flux_r[IEN][m] = wtot_r * ucon_r[IVY] * ucov_r[0] - bcon_r[IVY] * bcov_r[0];
      flux_r[IVX][m] = wtot_r * ucon_r[IVY] * ucov_r[1] - bcon_r[IVY] * bcov_r[1];
      flux_r[IVY][m] = wtot_r * ucon_r[IVY] * ucov_r[2] - bcon_r[IVY] * bcov_r[2];
      flux_r[IVZ][m] = wtot_r * ucon_r[IVY] * ucov_r[3] - bcon_r[IVY] * bcov_r[3];
      flux_r[IVY][m] += ptot_r;
      flux_r[IBY][m] = bcon_r[IVZ] * ucon_r[IVY] - bcon_r[IVY] * ucon_r[IVZ];
      flux_r[IBZ][m] = bcon_r[IVX] * ucon_r[IVY] - bcon_r[IVY] * ucon_r[IVX];
