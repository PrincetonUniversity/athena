...
//--- Step2.  Compute Roe-averaged state
    ...
    Real el,er,hroe;
    el = wli[IPR]*gm1_recip + 0.5*wli[IDN]*(SQR(wli[IVX]) + SQR(wli[IVY]) + SQR(wli[IVZ]));
    er = wri[IPR]*gm1_recip + 0.5*wri[IDN]*(SQR(wri[IVX]) + SQR(wri[IVY]) + SQR(wri[IVZ]));
