...
// i-direction
    ...
    if (order == 4) {
      ...
      // TODO(kfelker): assuming uniform mesh with dx1f=dx2f=dx3f,
      //                so this should factor out
      // TODO(kfelker): also, this may need to be dx1v,
      //                since Laplacian is cell-centered
      ...
      RiemannSolver(kl, ku, jl, ju, is, ie+1,
                    IVX, b1, wl_fc_, wr_fc_, flux_fc, e3x1, e2x1);
...
// j-direction
    ...
    if (pmb->block_size.nx2 > 1) {
      ...
      if (order == 4) {
        ...
        pmb->pcoord->LaplacianX2(wl, laplacian_l_fc, il, iu, js, je+1, kl, ku,
                                 0, NWAVE-1);
        pmb->pcoord->LaplacianX2(wr, laplacian_r_fc, il, iu, js, je+1, kl, ku,
                                 0, NWAVE-1);
...
// k-direction
    ...
    if (pmb->block_size.nx3 > 1) {
      ...
      if (order == 4) {
        ...
        pmb->pcoord->LaplacianX3(wl, laplacian_l_fc, il, iu, jl, ju, ks, ke+1,
                                 0, NWAVE-1);
        pmb->pcoord->LaplacianX3(wr, laplacian_r_fc, il, iu, jl, ju, ks, ke+1,
                                 0, NWAVE-1);
