...
void OutflowCellFieldInnerX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &bc,
                             int il, int iu, int jl, int ju, int kl, int ngh) {
  for (int n=0; n<NFIELD; ++n) {
    for (int k=1; k<=ngh; ++k) {
      for (int j=jl; j<=ju; ++j) {
        for (int i=il; i<=iu; ++i) {
          bc(n,kl-k,j,i) = bc(n,k,jl,i);
        }
      }
    }
  }
...
void OutflowCellFieldOuterX3(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &bc,
                             int il, int iu, int jl, int ju, int ku, int ngh) {
  for (int n=0; n<NFIELD; ++n) {
    for (int k=1; k<=ngh; ++k) {
      for (int j=jl; j<=ju; ++j) {
        for (int i=il; i<=iu; ++i) {
          bc(n,ku+k,j,i) = bc(n,k,ju,i);
        }
      }
    }
  }
