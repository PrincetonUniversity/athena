...
void RadBoundaryVariable::VacuumInnerX1(
  ...
  int &nang = pmb->prad->nang; // angles per octant
  ...
  for (int ifr=0; ifr<nfreq; ++ifr){
    for(int n=0; n<nang; ++n){
      int ang=ifr*nang+n;
      Real& miux=pmb->prad->mu(0,k,j,il,n);
      if(miux < 0.0){
...
void RadBoundaryVariable::VacuumOuterX1(
  ...
  for (int ifr=0; ifr<nfreq; ++ifr){
    for(int n=0; n<nang; ++n){
      int ang=ifr*nang+n;
      Real& miux=pmb->prad->mu(0,k,j,iu,n);
      if(miux > 0.0){
...
void RadBoundaryVariable::VacuumInnerX2(
  ...
  for (int ifr=0; ifr<nfreq; ++ifr){
    for(int n=0; n<nang; ++n){
      int ang=ifr*nang+n;
      Real& miuy=pmb->prad->mu(1,k,jl,i,n);
      if(miuy < 0.0){
...
void RadBoundaryVariable::VacuumOuterX2(
  ...
  for (int ifr=0; ifr<nfreq; ++ifr){
    for(int n=0; n<nang; ++n){
      int ang=ifr*nang+n;
      Real& miuy=pmb->prad->mu(1,k,ju,i,n);
      if(miuy > 0.0){
...
void RadBoundaryVariable::VacuumInnerX3(
  ...
  for (int ifr=0; ifr<nfreq; ++ifr){
    for(int n=0; n<nang; ++n){
      int ang=ifr*nang+n;
      Real& miuz=pmb->prad->mu(2,kl,j,i,n);
      if(miuz < 0.0){
...
void RadBoundaryVariable::VacuumOuterX3(
  ...
  for (int ifr=0; ifr<nfreq; ++ifr){
    for(int n=0; n<nang; ++n){
      int ang=ifr*nang+n;
      Real& miuz=pmb->prad->mu(2,ku,j,i,n);
      if(miuz > 0.0){
