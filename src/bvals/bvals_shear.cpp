	...
	for(int n=0; n<4; n++) {
	  if(send_inner_rank_[n] != -1) {
	    ...
	    if (send_inner_rank_[n] == Globals::my_rank) {// on the same process
          ...
          std::memcpy(pbl->pbval->recv_innerbuf_hydro_[n],
                  send_innerbuf_hydro_[n], send_innersize_hydro_[n]*ssize*sizeof(Real));
	  ...
	  if(send_outer_rank_[n] != -1) {
	    ...
	    if (send_outer_rank_[n] == Globals::my_rank) {// on the same process
          ...
          std::memcpy(pbl->pbval->recv_outerbuf_hydro_[n],
                  send_outerbuf_hydro_[n], send_outersize_hydro_[n]*ssize*sizeof(Real));
...
void BoundaryValues::FindShearBlock(void)
{
  ...
  if (shbb_.inner == true) { // if inner block
	...
	recv_innersize_hydro_[1] = je-js-joverlap_+1+NGHOST;
