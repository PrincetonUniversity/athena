...
//--------------------------------------------------------------------------------------
//! \fn int BoundaryValues::LoadHydroShearing(AthenaArray<Real> &src, Real *buf1, Real buf2, int joverlap)
//  \brief Load shearingbox hydro boundary buffers
void BoundaryValues::LoadHydroShearing(AthenaArray<Real> &src, Real *buf1, Real *buf2, int joverlap)
{
  MeshBlock *pmb=pmy_mblock_;
  Mesh *pmesh=pmb->pmy_mesh;

  int si = pmb->is-NGHOST;
  int ei = pmb->is-1;
  int sj = pmb->js;
  int ej = pmb->je-joverlap;
  int sk = pmb->ks;
  int ek = pmb->ke;
  ...
  // load [js:je-joverlap] of current meshblock for send[1]
  int p=0;
  BufferUtility::Pack4DData(src, buf2, 0, NHYDRO-1, si, ei, sj, ej, sk, ek, p);
  std::cout << "buf2[js:je-joverlap] size = " << p << std::endl;
  // send [je-(joverlap-1):je] of current meshblock if overlap for send[0]
  p=0;
  if (joverlap != 0) {
    sj = pmb->je-(joverlap-1);
    ej = pmb->je;
    BufferUtility::Pack4DData(src, buf1, 0, NHYDRO-1, si, ei, sj, ej, sk, ek, p);
    std::cout << "buf1[je-(joverlap)+1:je] size = " << p << std::endl;
  }
                                               ...
                                               const int nb)
{
  ...
  // set [js+joverlap:je] of current meshblock with recv[1]
  // set [js:js+(joverlap-1)] of current meshblock with recv[0]
