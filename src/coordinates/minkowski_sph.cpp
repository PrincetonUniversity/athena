...
//========================================================================================
//! \file minkowski_sph.cpp
//  \brief implements functions for Minkowski spacetime and spherical (t,r,theta,phi)
...
MinkowskiSph::MinkowskiSph(MeshBlock *pmb, ParameterInput *pin, bool flag)
  ...
  for (int i = il-ng; i <= iu+ng; ++i) {
    Real r_m = x1f(i);
    Real r_p = x1f(i+1);
    x1v(i) = 3.0/4.0 * (SQR(SQR(r_p)) - SQR(SQR(r_m))) / (r_p*SQR(r_p) - r_m*SQR(r_m));
  }
  ...
  if (pmb->pmy_mesh->multilevel && MAGNETIC_FIELDS_ENABLED) {
    for (int i = il-ng; i <= iu+ng; ++i) {
      Real r_m = x1f(i);
      Real r_p = x1f(i+1);
      x1s2(i) = x1s3(i) = 2.0/3.0 * (r_p*SQR(r_p) - r_m*SQR(r_m)) / (SQR(r_p) - SQR(r_m));
    }
    if (pmb->block_size.nx2 == 1) {
...
void MinkowskiSph::Edge1Length(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::Edge2Length(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &lengths) {
  #pragma omp simd
  for (int i = il; i <= iu; ++i) {
...
void MinkowskiSph::Edge3Length(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &lengths) {
  #pragma omp simd
  for (int i = il; i <= iu; ++i) {
...
// \Delta L = 1/3 \Delta(r^3) \sin\theta
Real MinkowskiSph::GetEdge1Length(const int k, const int j, const int i) {
  return coord_len1_i1_(i) * coord_len1_j1_(j);
}

// \Delta L = r^2 (-\Delta\cos\theta)
Real MinkowskiSph::GetEdge2Length(const int k, const int j, const int i) {
  return coord_len2_i1_(i) * coord_len2_j1_(j);
}

// \Delta L = r^2 \sin\theta \Delta\phi
Real MinkowskiSph::GetEdge3Length(const int k, const int j, const int i) {
...
void MinkowskiSph::CenterWidth2(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::CenterWidth3(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::Face1Area(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::Face2Area(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &areas) {
  #pragma omp simd
  for (int i = il; i <= iu; ++i)
...
void MinkowskiSph::Face3Area(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &areas) {
  #pragma omp simd
  for (int i = il; i <= iu; ++i)
...
// \Delta A = r^2 (-\Delta\cos\theta) \Delta\phi
Real MinkowskiSph::GetFace1Area(const int k, const int j, const int i) {
  return coord_area1_i1_(i) * coord_area1_j1_(j) * dx3f(k);
}

// \Delta A = 1/3 \Delta(r^3) \sin\theta \Delta\phi
Real MinkowskiSph::GetFace2Area(const int k, const int j, const int i) {
  return coord_area2_i1_(i) * coord_area2_j1_(j) * dx3f(k);
}

// \Delta A = 1/3 \Delta(r^3) (-\Delta\cos\theta)
Real MinkowskiSph::GetFace3Area(const int k, const int j, const int i) {
...
void MinkowskiSph::CellVolume(const int k, const int j, const int il, const int iu,
...
Real MinkowskiSph::GetCellVolume(const int k, const int j, const int i) {
...
void MinkowskiSph::AddCoordTermsDivergence(const Real dt, const AthenaArray<Real> *flux,
  ...
  for (int k = pmy_block->ks; k <= pmy_block->ke; ++k) {
    for (int j = pmy_block->js; j <= pmy_block->je; ++j) {
      ...
      for (int i = pmy_block->is; i <= pmy_block->ie; ++i) {
        ...
        Real s_1 = 0.5 * (d1_g_22*tt22 + d1_g_33*tt33);
...
void MinkowskiSph::CellMetric(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::Face1Metric(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &g, AthenaArray<Real> &g_inv) {
...
void MinkowskiSph::Face2Metric(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &g, AthenaArray<Real> &g_inv) {
...
void MinkowskiSph::Face3Metric(const int k, const int j, const int il, const int iu,
    AthenaArray<Real> &g, AthenaArray<Real> &g_inv) {
...
void MinkowskiSph::PrimToLocal1(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::PrimToLocal2(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::PrimToLocal3(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::FluxToGlobal1(const int k, const int j, const int il, const int iu,
...
void MinkowskiSph::FluxToGlobal2(const int k, const int j, const int il, const int iu,
    const AthenaArray<Real> &cons, const AthenaArray<Real> &bbx, AthenaArray<Real> &flux,
    AthenaArray<Real> &ey, AthenaArray<Real> &ez) {
...
void MinkowskiSph::FluxToGlobal3(const int k, const int j, const int il, const int iu,
    const AthenaArray<Real> &cons, const AthenaArray<Real> &bbx, AthenaArray<Real> &flux,
    AthenaArray<Real> &ey, AthenaArray<Real> &ez) {
...
void MinkowskiSph::RaiseVectorCell(Real a_0, Real a_1, Real a_2, Real a_3, int k, int j,
...
void MinkowskiSph::LowerVectorCell(Real a0, Real a1, Real a2, Real a3, int k, int j,
