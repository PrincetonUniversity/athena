...
fn = lambda ix: os.path.join(dir, 'advection_1d_cvg_Gaussian_P.out1.{ix}.athdf'.format(
    ix=str(ix).zfill(5)))
...
def ff(ix=0):
    ...
    plt.plot(tmp_f['x1v'].flatten(), tmp_f['u'].flatten(), '-b')
    plt.plot(tmp_f['x1v'].flatten(), tmp_f['uExact'].flatten(), '--r')
