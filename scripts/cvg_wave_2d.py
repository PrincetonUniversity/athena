...
filter_dataset = (
    ...
    lambda _: (_['Time'] != 0) and (_['Time'] != 0.5) and (_['Time'] != 1.0),
)
...
data = pIOCh.data_load(data_file_info, quantities=['wError', 'wU', 'wExact'])
...
def plot_max_abs(data):
    ...
    nx1, err = data_extract(data, Time='1.0', nghost=2, integrator='rk4',
                            meshblock_thread_partition=False)
    plt.loglog(nx1, err, 'or')

    nx1, err = data_extract(data, Time='1.0', nghost=4, integrator='rk4',
                            meshblock_thread_partition=False)
    plt.loglog(nx1, err, 'xr')


    nx1, err = data_extract(data, Time='1.0', nghost=2, integrator='rk3',
                            meshblock_thread_partition=False)
    plt.loglog(nx1, err, 'og')

    nx1, err = data_extract(data, Time='1.0', nghost=4, integrator='rk3',
                            meshblock_thread_partition=False)
