...
AthenaArray<Real> custom0, custom1;
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin)
{
  for(int k=ks; k<=ke; k++) {
    for(int j=js; j<=je; j++) {
      for(int i=is; i<=ie; i++) {
        // user_out_var(0,k,j,i) = M_enc;
        user_out_var(0,k,j,i) = custom0(i);
        user_out_var(1,k,j,i) = custom1(i);
      }
    }
  }
}
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
    ...
    custom0.NewAthenaArray(imax);
    custom1.NewAthenaArray(imax);
